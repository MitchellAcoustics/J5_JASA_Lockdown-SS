...
```{r}
sjPlot::plot_model(Pleasant.results$scaled.model, type="diag")
```
...
Eventful.results <- lm.model.building(target="ISOEventful", features=features, train = train, test = test, verbose=verbosity, pred=TRUE, plot=TRUE, scale=TRUE, exclude.features = c("N5"))
...
```{r}
sjPlot::plot_model(Eventful.results$model, type="diag")
