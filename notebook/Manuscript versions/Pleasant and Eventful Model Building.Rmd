...
data <- data %>% mutate_at(vars(c("ISOPleasant", "ISOEventful", features, "Natural", "Traffic", "Human")), funs(as.numeric))
...
When running the Linear Mixed Effects model feature reduction, it turns out that LocationID is not selected as a random effect! We therefore instead build the ISOEventful model as a standard linear model, with `lm()`. We also end up removing N5 due to multi collinearity (VIF) issues.
...
ss.data <- prelockdownData[c("Natural", "Traffic", "Human")]
ss.data <- drop_na(ss.data)
res <- NbClust::NbClust(ss.data, method='ward.D2')
factoextra::fviz_nbclust(res)

```

```{r}
ss.data <- prelockdownData[c("Natural", "Traffic", "Human", "LocationID")]
ss.data <- drop_na(ss.data)
ss.summ <- aggregate(ss.data[, 1:3], list(ss.data$LocationID), mean)
rownames(ss.summ) <- ss.summ[, 1]
ss.summ <- ss.summ[, 2:4]

nbc <- NbClust(ss.summ, method='ward.D2', max.nc=7, distance = 'manhattan')
fviz_nbclust(nbc)
```

```{r}
clust <- kmeans(ss.summ, centers=2)
# plot(ss.summ, col=clust$cluster)
fviz_cluster(clust, data=ss.summ, repel=TRUE)
```

```{r}
res.pca <- prcomp(ss.summ, scale=TRUE)
